  ...
  <div class="sideBar">
    ...
    <el-form  label-width="120px" label-position="left" :class="isOpenMenu? 'form': 'form-hidden'">
      <el-form-item class="label" label="Жанр">
        <el-select class="select" @update:modelValue="chooseGenre" :model-value="store.state.filters.genre_type" placeholder="Виберіть жанр">
          <el-option text-color="green" class="item-select" v-for="genre in store.state.optionsForGenre" :key="genre" :label="genre" :value="genre" />
        </el-select>
      </el-form-item>
      <el-form-item class="label" label="Рік випуску">
        <el-select class="select" @update:modelValue="chooseYear" :model-value="store.state.filters.year" placeholder="Виберіть рік випуску">
          <el-option v-for="year in store.state.optionsForYear" :key="year" :label="year" :value="year" />
        </el-select>
      </el-form-item>
      <el-form-item class="label" label="Рейтинг">
        <el-input :model-value="store.state.filters.rating_score" @input="chooseRate" />
      </el-form-item>
      <button-main :disable="!store.getters.isChangedFilters" label="Застосувати фільтр" @onClick="applyFilter" />
    </el-form>
  </div>
</template>
